...
type OwnProps = {
    ...
    isReadOnly: boolean;
    pluginPostTypes?: $TSFixMe;
    channelIsArchived: boolean;
    handleCardClick: $TSFixMeFunction;
...
type Props = OwnProps & typeof RhsRootPost.defaultProps;

export default class RhsRootPost extends React.PureComponent<Props, State> {
    static defaultProps = {
        commentCount: 0,
    };

    dotMenuRef: $TSFixMe;
    postHeaderRef: $TSFixMe;
    postRef: $TSFixMe;
    ...
    handleShortcutReactToLastPost = (isLastPost: $TSFixMe) => {
        if (isLastPost) {
            const {
                channelIsArchived,
                enableEmojiPicker,
            ...
            const isPostsFakeParentDeleted =
                post && post.type === Constants.PostTypes.FAKE_PARENT_DELETED;

            // Checking if rhs root comment is at scroll view of the user
            const boundingRectOfPostInfo =
                this.postHeaderRef.current.getBoundingClientRect();
            const isPostHeaderVisibleToUser =
                boundingRectOfPostInfo.top - 110 > 0 &&
    ...
    componentDidMount() {
        if (this.postRef.current) {
            this.postRef.current.addEventListener(
    ...
    componentWillUnmount() {
        ...
        if (this.postRef.current) {
            this.postRef.current.removeEventListener(
                A11yCustomEventTypes.ACTIVATE,
                this.handleA11yActivateEvent,
            );
            this.postRef.current.removeEventListener(
    ...
    componentDidUpdate(prevProps: Props) {
        ...
        if (this.state.a11yActive) {
            this.postRef.current.dispatchEvent(
    ...
    renderPostTime = (isEphemeral: $TSFixMe) => {
    ...
    handleActionsMenuOpened = (open: $TSFixMe) => {
    ...
    handleCommentClick = (open: $TSFixMe) => {
        this.setState({showDotMenu: open});
    };

    handleDotMenuOpened = (open: $TSFixMe) => {
        this.setState({showDotMenu: open});
    };
    ...
    handleFileDropdownOpened = (open: $TSFixMe) => {
        this.setState({fileDropdownOpened: open});
    };

    handlePostClick = (e: $TSFixMe) => {
    ...
    render() {
        ...
        const colorize =
            this.props.compactDisplay && this.props.colorizeUsernames;
        ...
        const postClass = classNames('post__body--transition', {
            'post--edited': PostUtils.isEdited(this.props.post),
        });
        ...
        let postFlagIcon;
        const showFlagIcon =
            !isEphemeral && !post.failed && !isSystemMessage && !isMobileView;
        if (showFlagIcon) {
            postFlagIcon = (
                <PostFlagIcon
                    location={Locations.RHS_ROOT}
        ...
        if (
            !isPostDeleted &&
            this.props.post.type !== Constants.PostTypes.FAKE_PARENT_DELETED
        ) {
            dotMenuContainer = (
                <div
                    ref={this.dotMenuRef}
